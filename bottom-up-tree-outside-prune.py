...
global all_nonterminals, lex_dict, spans_dict, substring_translations, stopwords, lm_model, weight_similarity, weight_binary_nt
...
def get_similarity(t_x, E_y):
    global weight_similarity, weight_binary_nt
    """
    ...
    for e_y in E_y:
        ...
        # sim_score = (weight_similarity * editdistance_prob(t_x.phrase, e_y.phrase)) + (weight_binary_nt * e_y.score)
    ...
    try:
        t_x.score = max_similarity_score + t_x.score
    except ValueError:
        pdb.set_trace()
...
def get_combinations(E_y, E_z, g_x):
    ...
    for e1, e2 in it.chain(it.product(E_y, E_z), it.product(E_z, E_y)):
        insert_lm_score, e_phrase = insert_stopword(e1, e2)  # returns best (score,phrase) with  LM cost
...
def outside_score_prune(T_x, ref):
    ...
    S_x = []
    for t_x in T_x:
        similarity_with_ref = meteor(t_x.phrase, ref) * weight_outside_similarity
        ...
        if len(en) < 25:
            ...
            for span in xrange(1, n):
                for i in xrange(0, n - span):
                    ...
                    # prune all non-terminals in a binary node using meteor similarly with reference
                    ...
                    if k == n - 1 and i == 0:
                        ...
                        T_x = get_human_reference(reference_root)
                    else:
